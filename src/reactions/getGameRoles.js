module.exports = {
    ...
    async execute(client, event, reaction, user) {
        ...
        const member = reaction.message.guild.members.fetch({ user: user.id });
        switch (event) {
            case 'messageReactionAdd':
                member.roles.add(memberRoles[reaction.emoji.name])
                break;
            case 'messageReactionRemove':
                member.roles.remove(memberRoles[reaction.emoji.name])
                break;
